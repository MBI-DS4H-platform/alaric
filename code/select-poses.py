...
def _load_npy(path: Path) -> np.ndarray:
    if path.name.endswith(".npy.zst"):
        try:
            ...
            raise ImportError("zstandard is required to read .npy.zst files") from exc
        with path.open("rb") as compressed:
            with zstd.ZstdDecompressor().stream_reader(compressed) as reader:
                return np.load(io.BytesIO(reader.read()))
    return np.load(path)
...
def _load_pose_indices(
    ...
    *,
    ...
    arr = _load_npy(path) if is_numpy else np.loadtxt(path)
    arr = np.asarray(arr)
    if arr.ndim == 0:
        arr = arr.reshape(1)
    if arr.ndim != 1:
...
def main(argv: list[str] | None = None) -> int:
    ...
    index_path = Path(args.pose_index_file)
